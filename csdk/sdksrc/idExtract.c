...
int icd_code_binary_search(const char *arr[], int low, int high, char *target)
{
    while (low <= high)
    {
        int mid = (high + low) / 2;
...
void get_HID_SID(char *buff, int *HID, int *SID, int *ICD_CODE)
{
    ...
    *ICD_CODE = -1;
    ...
    str_cpy(buff, icd_code_str, &str_index, ',');
#endif
    *ICD_CODE = icd_code_binary_search(icd_codes, 0, icd_code_nums, icd_code_str);
}
...
void id_extract(struct ID_node **result, int *r_size, int MPI_rank, int MPI_size)
{
    ...
    if (MPI_rank == 0)
    {
        ...
        // 发送
        int size_per_process = HADMID_size / MPI_size;

        // 保留本进程数据
        *r_size = size_per_process;
        *result = (struct ID_node *)malloc((*r_size) * sizeof(struct ID_node));
        for (i = 0; i < *r_size; i++)
        {
            (*result)[i].HADM_ID = HADMID[i].HADM_ID;
            (*result)[i].SUBJECT_ID = HADMID[i].SUBJECT_ID;
            (*result)[i].ICD_CODE = HADMID[i].ICD_CODE;
        }
        // 遍历, 给每个进程发送数据
        for (i = 1; i < MPI_size; i++)
        {
            int true_size = size_per_process;
            int start_index = i * size_per_process;
